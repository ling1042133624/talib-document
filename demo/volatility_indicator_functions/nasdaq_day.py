...
class FortyDaySMAStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.EMA = bt.talib.EMA(self.atr, timeperiod=14)
        self.EMA_long = bt.talib.EMA(self.atr, timeperiod=66)
        self.EMA.plotinfo.plotmaster = self.atr
        self.EMA_long.plotinfo.plotmaster = self.atr
    ...
    def indicators(self):
        ...
        # NATR和ATR差不多，只是变成百分比了 NATR = (ATR / 当前收盘价) * 100
        self.NATR = bt.talib.NATR(self.data.high, self.data.low, self.data.close, timeperiod=14)

        # TRANGE和ATR差不多，ATR对TRANGE做了平滑处理
        self.TRANGE = bt.talib.TRANGE(self.data.high, self.data.low, self.data.close, timeperiod=200)
        self.EMA = bt.talib.EMA(self.TRANGE, timeperiod=14)
        self.EMA_long = bt.talib.EMA(self.TRANGE, timeperiod=66)
        self.EMA.plotinfo.plotmaster = self.TRANGE
        self.EMA_long.plotinfo.plotmaster = self.TRANGE
        pass
